...
#define OEM_LOWER_LIMIT_TEMP	200
#define OEM_UPPER_LIMIT_TEMP	350
...
static int report_cc_based_soc(struct qpnp_bms_chip *chip)
{
	...
	chip->last_soc = oem_bms_correct_soc(soc);
	chip->last_soc = bound_soc(chip->last_soc);
	soc = chip->last_soc;
				...
				int vbat_uv, int ibat_ua, int batt_temp)
{
	...
	int max_voltage_uv = chip->max_voltage_uv;

	batt_terminal_uv = vbat_uv + (ibat_ua * chip->r_conn_mohm) / 1000;
	max_voltage_uv   = oem_vddmax_get() * 1000;

	if (chip->soc_at_cv == -EINVAL) {
		if (batt_terminal_uv >= max_voltage_uv - VDD_MAX_ERR) {
			chip->soc_at_cv = soc;
...
static int oem_set_battery_data(struct qpnp_bms_chip *chip);
...
static void backup_charge_cycle(struct qpnp_bms_chip *chip)
{
	...
	oem_param_cycles_backup(chip->charge_cycles, chip->charge_increase);
}
							...
							int chargecycles)
{
	...
	oem_param_fcc_backup((DIV_ROUND_UP(fcc_mah, chip->fcc_resolution)),
							(DIV_ROUND_UP(chargecycles, CHGCYL_RESOLUTION)),
							pos);
...
static void fcc_learning_config(struct qpnp_bms_chip *chip, bool start)
{
	...
	if (start) {
		...
		if (is_new_fcc_valid(new_fcc_uah, fcc_uah)) {
			if(OEM_LOWER_LIMIT_TEMP <= batt_temp
			&& batt_temp <= OEM_UPPER_LIMIT_TEMP) {
			update_fcc_learning_table(chip, new_fcc_uah,
					chip->charge_cycles, batt_temp);
				oem_bms_get_deteriorationstatus(&batt_deterioration_status,
						(calculate_fcc(chip, FCC_DEFAULT_TEMP) / 1000),
						(interpolate_fcc(chip->fcc_temp_lut, FCC_DEFAULT_TEMP))
						);
			}
		}
	}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_BMS_BATT_STATUS:
		val->intval = oem_bms_get_deteriorationstatus(&batt_deterioration_status,
						(calculate_fcc(chip, FCC_DEFAULT_TEMP) / 1000),
						(interpolate_fcc(chip->fcc_temp_lut, FCC_DEFAULT_TEMP))
						);
		break;
...
static void load_shutdown_data(struct qpnp_bms_chip *chip)
{
	...
	if (oem_option_item1_bit6) {
		shutdown_soc = calculate_soc_from_voltage(chip);
	}
	else {
...
static void oem_bms_set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	for (i = 0; i < chip->min_fcc_learning_samples; i++) {
		if (oem_param_cycles.fcc[i] == 0 || (oem_param_cycles.fcc[i] == 0xFF && oem_param_cycles.chgcyl[i] == 0xFF)) {
			/* FCC invalid/not present */
			chip->fcc_learning_samples[i].fcc_new = 0;
			chip->fcc_learning_samples[i].chargecycles = 0;
		} else {
			...
			chip->fcc_sample_count++;
			chip->fcc_learning_samples[i].fcc_new =
						oem_param_cycles.fcc[i] * chip->fcc_resolution;
			chip->fcc_learning_samples[i].chargecycles =
						oem_param_cycles.chgcyl[i] * CHGCYL_RESOLUTION;
		}
	}
}
#endif
...
static bool oem_bms_uim_status_fixed = false;
static int oem_set_battery_data(struct qpnp_bms_chip *chip)
{
	struct bms_battery_data *batt_data;
	static bool initialized = false;

	pr_debug("initialized = %d, oem_bms_uim_status_fixed = %d \n",
	          initialized, oem_bms_uim_status_fixed);
	if(!initialized && oem_bms_uim_status_fixed)
	{
		if (false == oem_is_uim_dete()) {
			batt_data = &oem_batt_data_nouim;

			pr_debug("Battery data Update for NoUIM Mode \n");
			chip->fcc_mah = batt_data->fcc;
			chip->fcc_temp_lut = batt_data->fcc_temp_lut;
			chip->fcc_sf_lut = batt_data->fcc_sf_lut;
			chip->pc_temp_ocv_lut = batt_data->pc_temp_ocv_lut;
			...
			pr_debug("Battery data NOT Update. UIM is Inserted. \n");
		}
		initialized = true;
	}
...
void oem_bms_uim_status_notify(void)
{
	oem_bms_uim_status_fixed = true;
	pr_debug("OEM BMS UIM Status Fixed!! \n");
}
...
void oem_bms_reset_cc(void)
{
	...
	if (get_battery_status(the_chip) == POWER_SUPPLY_STATUS_FULL) {
		count += (OEM_HKADC_MONITOR_TIME_MS /1000);
		if(OEM_1_SECOND_TO_10_MINUTES <= count) {
			count = 0;
