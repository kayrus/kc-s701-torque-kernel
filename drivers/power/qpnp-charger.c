...
#define INITIAL_DELTA_MV	10
static unsigned int oem_recharge_delta_mv = 140;
#define FACTORY_CHG_VOLT_MV	3970
extern int oem_option_item1_bit6;

static int oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_INIT;

enum oem_recharge_req_flag_type {
	OEM_RECHARGE_INIT = 0,
	OEM_RECHARGE_START
};
static int oem_chg_recherge_req_flg = OEM_RECHARGE_INIT;
...
#ifdef CONFIG_OEM_HKADC_USB_TM
static int thermel_engine_value = 0;

static void oem_chg_usb_tm_level_set(struct power_supply *psy);
#endif
...
{
	if (enable == 0) {
		__cancel_delayed_work(&chip->charge_en_work);
	}
...
{
	if ( ( (chip->chg_done) && (OEM_RECHARGE_START != oem_chg_recherge_req_flg) ) || (oem_chg_vbatdet_change_flg == true) ) {
	...
	} else if (false == oem_is_uim_dete() && (!oem_option_item1_bit6)) {
...
static void
oem_charge_en_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct qpnp_chg_chip *chip = container_of(dwork,
				struct qpnp_chg_chip, charge_en_work);

	pr_debug("oem_charge_en_work");
...
qpnp_arb_stop_work(struct work_struct *work)
{
	...
	if (!chip->chg_done)
		...
		schedule_delayed_work(&chip->charge_en_work,
...
{
	if (false == oem_is_uim_dete() && (!oem_option_item1_bit6)) {
		qpnp_chg_vddmax_and_trim_set(chip, oem_param_charger.uim_undete_chg, chip->delta_vddmax_mv);
		return;
	}
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	oem_chg_usb_usbin_valid_handler(usb_present);
	...
	if (chip->usb_present ^ usb_present) {
		...
		oem_chg_vbatdet_change_flg = false;
		oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_INIT;
...
#define OEM_H_CHG_ON_EN_THRESHOLD 1500
static irqreturn_t
		...
		if (fastchg_on) {
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				...
				if (ret.intval == POWER_SUPPLY_TYPE_USB_DCP) {
					...
					if ((ret.intval / 1000) == OEM_H_CHG_ON_EN_THRESHOLD) {
...
bool oem_is_usb_overvolutage_bit = 0;
...
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	...
	if (!rc) {
		if ( 0x40 == (usbin_valid_rt_sts & USB_OVERVOLUTAGE_BIT) ) {
			oem_is_usb_overvolutage_bit = 1;
		}else {
			oem_is_usb_overvolutage_bit = 0;
		}
	}
	...
	if ((qpnp_chg_is_usb_chg_plugged_in(chip) ||
		qpnp_chg_is_dc_chg_plugged_in(chip)) && (oem_after_warmcool_recharge_flag == OEM_AFTER_WARMCOOL_RECHARGING)) {
		pr_debug("Dummy Recharging \n");
		return POWER_SUPPLY_STATUS_FULL;
	}
	rc = qpnp_chg_read(chip, &chgr_sts, INT_RT_STS(chip->chgr_base), 1);
	if (rc) {
	...
	if (oem_chg_wpc_get_init_status() == 0) {
		return POWER_SUPPLY_STATUS_CHARGING;
	}
...
qpnp_batt_external_power_changed(struct power_supply *psy)
{
	...
	if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
		...
		if (oem_chg_get_batt_property(POWER_SUPPLY_PROP_OEM_CHARGE_CONNECT_STATE)
			== POWER_SUPPLY_OEM_CONNECT_CHARGE_STAND_1) {
			qpnp_chg_iusbmax_set(chip, ret.intval / 1000);
		} else if (ret.intval <= 2 && !chip->use_default_batt_values &&
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_USB_THERM:
		val->intval = oem_hkadc_usb_therm_value();
		break;
#endif
	default:
		return -EINVAL;
...
//		chg_current = min(chg_current, (unsigned int)oem_param_charger.i_chg_nouim);
...
#define VBATDET_MAX_ERR_MV	90
#define VBATDET_MAX_ERR_COOL_MV	190
#define VBATDET_MAX_ERR_WARM_MV	190
static void
	...
	if ((batt_sts & BAT_FET_ON_IRQ) && (chg_sts & FAST_CHG_ON_IRQ
		...
		if (vbat_lower_than_vbatdet && vbat_mv <
			...
			if (vbat_low_count >= CONSECUTIVE_COUNT) {
				...
				if(oem_after_warmcool_recharge_flag == OEM_AFTER_WARMCOOL_RECHARGING) {
					oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_CHARGE_DONE;
					pr_debug("Stop dummy RECHARGING\n");
					power_supply_changed(&chip->batt_psy);
				}
		...
		if (!(buck_sts & VDD_LOOP_IRQ)) {
			...
			if (count == CONSECUTIVE_COUNT) {
				...
				if ((chip->bat_is_cool == true || chip->bat_is_warm == true) &&
					(oem_after_warmcool_recharge_flag <= OEM_AFTER_WARMCOOL_CHARGE_DONE_ON_NOT_NORMAL) ) {
					oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_CHARGE_DONE_ON_NOT_NORMAL;
				} else {
					oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_CHARGE_DONE;
				}
...
#ifdef CONFIG_OEM_HKADC_USB_TM
		pr_debug("value is evacuated temporarily. :%d\n",val->intval);
		thermel_engine_value = val->intval;
		if (oem_hkadc_usb_therm_error_state()) {
			break;
		}
#endif
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	if (oem_option_item1_bit6) {
		chip->max_voltage_mv	= FACTORY_CHG_VOLT_MV;
		chip->warm_bat_mv	= FACTORY_CHG_VOLT_MV;
		chip->cool_bat_mv	= FACTORY_CHG_VOLT_MV;
		pr_info("max_voltage_mv:%d warm_bat_mv:%d cool_bat_mv:%d \n",
			chip->max_voltage_mv, chip->warm_bat_mv, chip->cool_bat_mv);
	}
	/*
	...
	INIT_DELAYED_WORK(&chip->charge_en_work, oem_charge_en_work);
...
#ifdef CONFIG_OEM_HKADC_USB_TM
	oem_hkadc_usb_tm_init(&chip->batt_psy);
#endif
...
qpnp_charger_remove(struct spmi_device *spmi)
{
	...
	cancel_delayed_work_sync(&chip->charge_en_work);
...
static void oem_chg_usb_tm_level_set(struct power_supply *psy)
{
	...
	if (oem_hkadc_usb_therm_error_state()) {
		pr_err("value of the anomaly temperature is set.\n");
		set_level = chip->thermal_levels - 1;
	}else{
		pr_debug("evacuated value is set. :%d\n",thermel_engine_value);
		set_level = thermel_engine_value;
	}
	
	qpnp_batt_system_temp_level_set(chip, set_level);
}
#endif
...
int oem_chg_is_thermal_limitation(void)
{
	return (the_chip->therm_lvl_sel >= (the_chip->thermal_levels - 1));
}
EXPORT_SYMBOL(oem_chg_is_thermal_limitation);

#ifdef CONFIG_OEM_HKADC_USB_TM
void oem_hkadc_usb_tm_level_set(struct power_supply *psy)
{
	if(oem_option_item1_bit3){
		return;
	}
	
	oem_chg_usb_tm_level_set(psy);
}
EXPORT_SYMBOL(oem_hkadc_usb_tm_level_set);
#endif
...
unsigned int oem_vddmax_get(void)
{
	...
	if(false == oem_is_uim_dete()) {
		vddmax = oem_param_charger.uim_undete_chg;
	}
	else if(the_chip->bat_is_cool) {
		vddmax = the_chip->cool_bat_mv;
	}
	else if(the_chip->bat_is_warm) {
		vddmax = the_chip->warm_bat_mv;
	}
	else {
		vddmax = the_chip->max_voltage_mv;
	}
	return vddmax;
}
...
EXPORT_SYMBOL(oem_clear_oem_chg_vbatdet_change_flag);


int oem_get_status_oem_after_warmcool_recharge_flag(void)
{
	return oem_after_warmcool_recharge_flag;
}
EXPORT_SYMBOL(oem_get_status_oem_after_warmcool_recharge_flag);

void oem_clear_oem_after_warmcool_recharge_flag(void)
{
	oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_INIT;
}
EXPORT_SYMBOL(oem_clear_oem_after_warmcool_recharge_flag);

void oem_recharge_request(void)
{
	if ( (qpnp_chg_is_usb_chg_plugged_in(the_chip) || qpnp_chg_is_dc_chg_plugged_in(the_chip)) 
		&& !the_chip->charging_disabled ) {
		oem_chg_recherge_req_flg = OEM_RECHARGE_START;
		qpnp_chg_set_appropriate_vbatdet(the_chip);
		oem_chg_recherge_req_flg = OEM_RECHARGE_INIT;
		qpnp_chg_charge_en(the_chip, !the_chip->charging_disabled);
		oem_after_warmcool_recharge_flag = OEM_AFTER_WARMCOOL_RECHARGING;
		pr_debug("Recharge start\n");
	}
}
EXPORT_SYMBOL(oem_recharge_request);

int oem_chg_get_vusb_averaged(int sample_count)
{
	return get_vusb_averaged(the_chip, sample_count);
}
EXPORT_SYMBOL(oem_chg_get_vusb_averaged);
