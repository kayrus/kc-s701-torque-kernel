...
#define OEM_STATE_SLEEP_BIT	(1<<0) /* suspend(1) or resume(0) */
#define OEM_STATE_SWITCH_BIT	(1<<1) /* patent on(1) or off(0) */

#define OEM_LIMIT_EVENT_NUM_IN_CALL	(2)/* limit event num during a call & proximity detection */
...
MODULE_FIRMWARE(CY_FW_FILE_01_NAME);
MODULE_FIRMWARE(CY_FW_FILE_02_NAME);
MODULE_FIRMWARE(CY_CFG_FILE_01_NAME);
...
/* change finger/glove mode */
static int cyttsp4_set_touchmode_enable(struct cyttsp4_core_data *cd, u32 mode);
...
static int cyttsp4_set_touchmode_enable(struct cyttsp4_core_data *cd, u32 mode)
{
	...
	rc = cyttsp4_set_parameter(cd, CY_RAM_ID_TOUCHMODE_ENABLED, 1, mode);
	if (rc < 0) {
...
static void cyttsp4_detect_finger_(struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	...
	cd->touchmode_enable = CY_CMD_TOUCHMODE_FINGER_ONLY;
	schedule_work(&cd->touchmode_work);
}

/* detect suppression */
static void cyttsp4_detect_large_(struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
...
{
	struct cyttsp4_core_data *cd =
		container_of(work, struct cyttsp4_core_data, touchmode_work);
	...
	cyttsp4_set_touchmode_enable(cd, cd->touchmode_enable);
}
/* change finger/glove mode */
...
enum {
	CY_FT_MODE_UNKNOWN = -2,
	CY_FT_MODE_I2C_ERROR = -1,
	CY_FT_MODE_BOOTLOADER = 0,
	CY_FT_MODE_SYSINFO,
	CY_FT_MODE_CAT,
	CY_FT_MODE_OPERATING,
};
enum {
	CY_FT_BUFCTRL_CLROFS,
	CY_FT_BUFCTRL_SETDATA,
	CY_FT_BUFCTRL_SETRET,
};
#define CY_FT_BUFSIZE (2048)
/* return mode: Bootloader=0 SystemInformation=1 CaT=2 Operating=3 */
static int cyttsp4_ft_mode_check(struct cyttsp4_core_data *cd)
{
	u8 mode[2];
	int rc = 0;
	...
	rc = cyttsp4_adap_read(cd, CY_REG_BASE, &mode, sizeof(mode));
	if (rc) {
		printk("%s: Fail read adapter r=%d\n", __func__, rc);
		return CY_FT_MODE_I2C_ERROR;
	}

	if (IS_BOOTLOADER(mode[0], mode[1])) {
		return CY_FT_MODE_BOOTLOADER;
	}

	switch(GET_HSTMODE(mode[0])) {
	case 1:
		rc = CY_FT_MODE_SYSINFO;
		break;
	case 2:
		rc = CY_FT_MODE_CAT;
		break;
	case 0:
		rc = CY_FT_MODE_OPERATING;
		break;
	default:
		rc = CY_FT_MODE_UNKNOWN;
		break;
	}
...
static int cyttsp4_ft_mode_recovery(struct cyttsp4_core_data *cd, int prev_mode)
{
	int rc = 0;

	switch(prev_mode) {
	case CY_FT_MODE_SYSINFO:
		rc = set_mode(cd, CY_MODE_SYSINFO);
		break;
	case CY_FT_MODE_OPERATING:
		rc = set_mode(cd, CY_MODE_OPERATIONAL);
		break;
	default:
		break;
...
static int cyttsp4_ft_get_electrodes(struct cyttsp4_core_data *cd ,u8 *x, u8 *y)
{
	u8 return_buf[2] = {0};
	int rc = 0;

	if(NULL != cd->sysinfo.si_ptrs.pcfg) {
		*x = cd->sysinfo.si_ptrs.pcfg->electrodes_x;
		*y = cd->sysinfo.si_ptrs.pcfg->electrodes_y;
	}
	else {
		rc = cyttsp4_adap_read(cd, 0x08, return_buf, 2);
		if(rc < 0) {
			goto exit;
		}
		rc = cyttsp4_adap_read(cd, return_buf[1], return_buf, 2);
		if(rc < 0) {
			goto exit;
		}
		*x = return_buf[0];
...
static int cyttsp4_ft_buf_ctrl(int ctrl, u8 *dst, u8 *src, int size)
{
	static int ofs = 1;
	int rc = 0;

	switch(ctrl) {
	case CY_FT_BUFCTRL_CLROFS:
		ofs = 1;
		break;
	case CY_FT_BUFCTRL_SETDATA:
		if(CY_FT_BUFSIZE - 1 <= ofs + size) {
			printk("%s: sum of result data size over buffer size remaining=%d\n",__func__,CY_FT_BUFSIZE - ofs);
			rc = -2;
		}
		else {
			memcpy(dst + ofs, src, size);
			ofs += size;
		}
		break;
	case CY_FT_BUFCTRL_SETRET:
		dst[0] = src[0];
		break;
	default:
...
static int cyttsp4_ft_error_check(u8 *retbuf, int ret)
{
	int rc = 0;

	if (ret || (0x00 != retbuf[0])) {
		rc = -1;
	}
	return rc;
}

static int cyttsp4_ft_get_self_test_result(struct cyttsp4_core_data *cd,
						int ofs, int len, u8 testid, u8 *retbuf)
{
	u8 command_buf[CY_CMD_CAT_GET_SELFTEST_RES_CMD_SZ] = {0};
	int rc = 0;

	command_buf[0] = CY_CMD_CAT_GET_SELF_TEST_RESULT;
	command_buf[1] = (u8)((ofs & 0x0000FF00) >> 8);
	command_buf[2] = (u8)(ofs & 0x000000FF);
	command_buf[3] = (u8)((len & 0x0000FF00) >> 8);
	command_buf[4] = (u8)(len & 0x000000FF);
	command_buf[5] = testid;

	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			command_buf, CY_CMD_CAT_GET_SELFTEST_RES_CMD_SZ,
			retbuf, len + CY_CMD_CAT_GET_SELFTEST_RES_RET_SZ,
			CY_COMMAND_COMPLETE_TIMEOUT);

	return rc;
}

static int cyttsp4_ft_cat_retrieve_data_structure(struct cyttsp4_core_data *cd,
							int ofs, int len, u8 dataid, u8 *retbuf)
{
	u8 command_buf[CY_CMD_CAT_RETRIEVE_DATA_STRUCT_CMD_SZ] = {0};
	int rc = 0;
	...
	command_buf[3] = (u8)((len & 0x0000FF00) >> 8);
	command_buf[4] = (u8)(len & 0x000000FF);
	command_buf[5] = dataid;

	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			command_buf, CY_CMD_CAT_RETRIEVE_DATA_STRUCT_CMD_SZ,
			retbuf, len + CY_CMD_CAT_RETRIEVE_DATA_STRUCT_RET_SZ,
			CY_COMMAND_COMPLETE_TIMEOUT);

	return rc;
}

static int cyttsp4_ft_cat_retrieve_panel_scan(struct cyttsp4_core_data *cd,
						int ofs, int len, u8 dataid, u8 *retbuf)
{
	u8 command_buf[CY_CMD_CAT_RETRIEVE_PANEL_SCAN_CMD_SZ] = {0};
	int rc = 0;
	...
	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			command_buf, CY_CMD_CAT_RETRIEVE_PANEL_SCAN_CMD_SZ,
			retbuf, len * 2 + CY_CMD_CAT_RETRIEVE_PANEL_SCAN_RET_SZ,
			CY_COMMAND_COMPLETE_TIMEOUT);

	return rc;
}

static int cyttsp4_ft_exec_short_test(struct cyttsp4_core_data *cd, u8 *retdata)
{
	u8 command_buf[6] = {0};
	u8 return_buf[0x0f + CY_CMD_CAT_GET_SELFTEST_RES_RET_SZ] = {0};
	int rc = 0;
	int prev_mode;

	cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_CLROFS, NULL, NULL, 0);
...
/* STEP00 hw reset */
	rc = cyttsp4_startup(cd);
	if (rc < 0) {
		printk("%s: HW reset failed r=%d\n",__func__, rc);
		return_buf[0] = 0xFF;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
	}

/* STEP01,02 */
	prev_mode = cyttsp4_ft_mode_check(cd);
	if(prev_mode < 0) {
		return_buf[0] = 0x01;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
	}
	else if(prev_mode != CY_FT_MODE_CAT) {
		rc = set_mode(cd, CY_MODE_CAT);
		if (rc < 0) {
			dev_err(cd->dev, "%s: failed to set mode to CaT rc=%d\n",
				__func__, rc);
...
/* STEP03 */
	command_buf[0] = CY_CMD_CAT_RUN_SELF_TEST;
	command_buf[1] = 0x04;
	command_buf[2] = 0x00;
	...
	if(cyttsp4_ft_error_check(return_buf, rc)) {
		printk("%s: Error short test ret=%02x %02x %02x\n",__func__,
...
/* STEP04 */
	memset(return_buf,0x00,sizeof(return_buf));

	rc = cyttsp4_ft_get_self_test_result(cd, 0, 0x0f, 0x04, return_buf);
	if (rc) {
		dev_err(cd->dev, "%s: Error executing command r=%d\n",
			__func__, rc);
		return_buf[0] = 0x04;
	...
	cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETDATA, retdata, return_buf, 0x0f + CY_CMD_CAT_GET_SELFTEST_RES_RET_SZ);

	return_buf[0] = 0x0;
	cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
exit:
	cyttsp4_ft_mode_recovery(cd,prev_mode);	
	return rc;
}

static int cyttsp4_ft_exec_open_test(struct cyttsp4_core_data *cd, u8 *retdata)
{
	u8 command_buf[6] = {0};
	u8 return_buf[4] = {0};
	u8 *return_buf_v;
	u8 electrodes_x,electrodes_y;
	u16 read_len = 0;
	int rc = 0;
	int prev_mode;
...
/* STEP01,02 */
	...
	else if(prev_mode != CY_FT_MODE_SYSINFO) {
		rc = set_mode(cd, CY_MODE_SYSINFO);
		if (rc < 0) {
			dev_err(cd->dev, "%s: failed to set mode to SystemInformation rc=%d\n",
				__func__, rc);
			return_buf[0] = 0x02;
			cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
			goto exit;
...
/* STEP03-06 */
	rc = cyttsp4_ft_get_electrodes(cd, &electrodes_x, &electrodes_y);
	if(rc < 0) {
		return_buf[0] = 0x03;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
	}
	read_len = electrodes_x * electrodes_y + 1;
...
/* STEP08 */
	...
	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			...
			CY_COMMAND_COMPLETE_TIMEOUT);
	if(cyttsp4_ft_error_check(return_buf, rc)) {
		printk("%s: Error open test ret=%02x %02x %02x\n",__func__,
					return_buf[0],return_buf[1],return_buf[2]);
		return_buf[0] = 0x08;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
...
/* STEP09 */
	return_buf_v = (u8 *)kmalloc(read_len + CY_CMD_CAT_RETRIEVE_DATA_STRUCT_RET_SZ, GFP_KERNEL);

	rc = cyttsp4_ft_get_self_test_result(cd, 0, read_len, 0x03, return_buf_v);
	if (rc) {
		dev_err(cd->dev, "%s: Error executing command r=%d\n",
			__func__, rc);
		return_buf[0] = 0x09;
	...
	cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETDATA, retdata, return_buf_v,
				read_len + CY_CMD_CAT_RETRIEVE_DATA_STRUCT_RET_SZ);
...
static int cyttsp4_ft_exec_calibration_test(struct cyttsp4_core_data *cd, u8 *retdata)
{
	u8 command_buf[6] = {0};
	u8 return_buf[2] = {0};
	u8 *return_buf_v;
...
/* STEP03a */
	...
	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			...
			CY_COMMAND_COMPLETE_TIMEOUT);
	if(cyttsp4_ft_error_check(return_buf, rc)) {
		printk("%s: Error calibration test 03a ret=%02x\n",__func__,
					return_buf[0]);
		return_buf[0] = 0x3a;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
...
/* STEP03b */
	...
	rc = cyttsp4_exec_cmd(cd, CY_MODE_CAT,
			...
			CY_COMMAND_COMPLETE_TIMEOUT);
	if(cyttsp4_ft_error_check(return_buf, rc)) {
		printk("%s: Error calibration test 03b test ret=%02x\n",__func__,
					return_buf[0]);
		return_buf[0] = 0x3b;
		cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
		goto exit;
...
/* STEP04a,4b */
	for(dataid = 0; dataid < 2; dataid++) {
		read_len = 0x150;
		if(1 == dataid) {
			read_len = electrodes_x + electrodes_y + 2;
		}
		return_buf_v = (u8 *)kmalloc(read_len + CY_CMD_CAT_RETRIEVE_DATA_STRUCT_RET_SZ, GFP_KERNEL);
		memset(return_buf_v,0x00,read_len + CY_CMD_CAT_RETRIEVE_DATA_STRUCT_RET_SZ);

		rc = cyttsp4_ft_cat_retrieve_data_structure(cd, 0, read_len, dataid, return_buf_v);
		if (rc) {
			dev_err(cd->dev, "%s: Error executing command r=%d\n",
				__func__, rc);
			return_buf[0] = 0x4a + (u8)dataid;
			cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETRET, retdata, return_buf, 1);
			kfree(return_buf_v);
			goto exit;
		}
...
static int cyttsp4_ft_exec_initialization_test(struct cyttsp4_core_data *cd, u8 *retdata)
{
	u8 command_buf[6] = {0};
	u8 return_buf[2] = {0};
	u8 *return_buf_v;
	...
	int dataid;
	int i;
	int max_r_len = cd->max_xfer - CY_CMD_CAT_RETRIEVE_PANEL_SCAN_RET_SZ;
	int max_elem_num = max_r_len / 2;
	int act_elem_num = 0;	
...
/* STEP05*/
	for(dataid = 0; dataid <= 5; dataid++) {
		if(dataid < 3)
			read_len = electrodes_x * electrodes_y;
		else
			read_len = electrodes_x + electrodes_y;
		
		return_buf_v = (u8 *)kmalloc(cd->max_xfer, GFP_KERNEL);

		for(i = 0; i <= (read_len / max_elem_num); i++) {
			memset(return_buf_v,0x00,cd->max_xfer);

			command_buf[1] = (u8)(((max_r_len / 2) * i) >> 8);
			command_buf[2] = (u8)(((max_r_len / 2) * i) & 0x000000FF);
			act_elem_num = max_elem_num;
			if((read_len - (max_elem_num * i)) <= max_elem_num) {
				act_elem_num = read_len - (max_elem_num * i);
			}
			rc = cyttsp4_ft_cat_retrieve_panel_scan(cd,
				max_elem_num * i, act_elem_num, dataid, return_buf_v);
			if (rc) {
				dev_err(cd->dev, "%s: Error executing command r=%d\n",
					__func__, rc);
				return_buf[0] = 0x05;
			...
			cyttsp4_ft_buf_ctrl(CY_FT_BUFCTRL_SETDATA, retdata,
				return_buf_v + CY_CMD_CAT_RETRIEVE_PANEL_SCAN_RET_SZ, (act_elem_num * 2));
...
/* FACTORY TEST */
static void cyttsp4_suspend_with_lcd(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	...
	cyttsp4_core_rt_suspend(dev);
}

static void cyttsp4_resume_with_lcd(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	u8 command_buf[2] = {0};
	u8 return_buf[2] = {0};
	...
	call_atten_cb(cd, CY_ATTEN_RESUME, 0);
	cyttsp4_core_rt_resume(dev);

	/* initialize touch info(calibration) when resume */
	rc = set_mode(cd, CY_MODE_CAT);
	if (rc < 0) {
		dev_err(cd->dev, "%s: failed to set mode to CAT rc=%d\n",
			__func__, rc);
...
/* for status management */
static int cyttsp4_change_state(struct device *dev, int state)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	...
	/* change to glove mode when end call */
	if(0 != (old_state & OEM_STATE_SWITCH_BIT)) {
		cd->touchmode_enable = CY_CMD_TOUCHMODE_AUTO_GLOVE;
		cyttsp4_set_touchmode_enable(cd, cd->touchmode_enable);
	}

	if((0 != (old_state & OEM_STATE_SLEEP_BIT)
	&&  0 == (old_state & OEM_STATE_SWITCH_BIT))
	|| (0 != (cd->oem_state & OEM_STATE_SLEEP_BIT)
	&&  0 == (cd->oem_state & OEM_STATE_SWITCH_BIT))) {
		/* SLEEP_BIT | SWITCH_BIT */
		switch(cd->oem_state) {
		case 0                   | 0:
		case 0                   | OEM_STATE_SWITCH_BIT:
		case OEM_STATE_SLEEP_BIT | OEM_STATE_SWITCH_BIT:
			enable_irq_wake(cd->irq);
			cyttsp4_resume_with_lcd(dev);
			break;
		case OEM_STATE_SLEEP_BIT | 0:
			disable_irq_wake(cd->irq);
			cyttsp4_suspend_with_lcd(dev);
			break;
		default:
			printk("%s: unknown state 0x%x\n",__func__,cd->oem_state);
			break;
		}
	}
...
/* for status management */
/* get touch count */
static int cyttsp4_diag_get_touch_count(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	u8 tt_stat = si->xy_mode[si->si_ofs.tt_stat_ofs];
	return GET_NUM_TOUCH_RECORDS(tt_stat);
} 

static long cyttsp4_ts_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	struct cyttsp4_core_data *cd = (struct cyttsp4_core_data *)file->private_data;
	struct device *dev = cd->dev;
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	...
	case IOCTL_DIAG_SHORT_TEST:
		retdata = (u8 *)kmalloc(CY_FT_BUFSIZE,GFP_KERNEL);
		if(NULL == retdata) {
			rc = -EFAULT;
			dev_err(dev, "%s: fail allocate memory\n", __func__);
			goto done;
		}
		memset(retdata,0,CY_FT_BUFSIZE);
		cyttsp4_ft_exec_short_test(cd,retdata);
		if (copy_to_user((void __user *)arg, retdata,CY_FT_BUFSIZE)) {
			rc = -EFAULT;
	...
	case IOCTL_DIAG_OPEN_TEST:
		...
		memset(retdata,0,CY_FT_BUFSIZE);
		cyttsp4_ft_exec_open_test(cd,retdata);
		if (copy_to_user((void __user *)arg, retdata,CY_FT_BUFSIZE)) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_to_user error\n", __func__);
		}
	...
	case IOCTL_DIAG_CALIB_TEST:
		...
		memset(retdata,0,CY_FT_BUFSIZE);
		cyttsp4_ft_exec_calibration_test(cd,retdata);
		if (copy_to_user((void __user *)arg, retdata,CY_FT_BUFSIZE)) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_to_user error\n", __func__);
		}
	...
	case IOCTL_DIAG_INIT_TEST:
		...
		memset(retdata,0,CY_FT_BUFSIZE);
		cyttsp4_ft_exec_initialization_test(cd,retdata);
		if (copy_to_user((void __user *)arg, retdata,CY_FT_BUFSIZE)) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_to_user error\n", __func__);
		}
		kfree(retdata);
		break;
	case IOCTL_SUSPEND_RESUME:
		pr_notice("%s: IOCTL_SUSPEND_RESUME\n", __func__);
		if (copy_from_user(&request_stat, (void __user *)arg, sizeof(bool))) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_from_user error\n", __func__);
			goto done;
		}
		if (!request_stat) {
			pr_notice("%s: IOCTL_SUSPEND_RESUME -> Resume!\n", __func__);
			mutex_lock(&cd->state_lock);
			cyttsp4_change_state(dev, (cd->oem_state & ~OEM_STATE_SLEEP_BIT));
			mutex_unlock(&cd->state_lock);
		}
		else {
			pr_notice("%s: IOCTL_SUSPEND_RESUME -> Suspend!\n", __func__);
			mutex_lock(&cd->state_lock);
	...
	case IOCTL_SET_SENSOR_SWITCH:
		dev_dbg(dev, "%s: IOCTL_SET_SENSOR_SWITCH\n", __func__);
	...
	case IOCTL_MULTI_GET:
		dev_dbg(dev, "%s: IOCTL_MULTI_GET\n", __func__);
		memset(&diag_data, 0, sizeof(struct ts_diag_type));
		diag_data.diag_count = cyttsp4_diag_get_touch_count(cd);
		if(copy_to_user((void __user *)arg, &diag_data, sizeof(struct ts_diag_type))) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_to_user error\n", __func__);
			goto done;
		}
		break;
	/* limit event num during a call & proximity detection */
	case IOCTL_CHANGE_MODE:
		dev_dbg(dev, "%s: IOCTL_CHANGE_MODE\n", __func__);
		if (copy_from_user(&mode_f, (void __user *)arg, sizeof(struct ts_mode_flag))) {
			rc = -EFAULT;
			dev_err(dev, "%s: copy_from_user error\n", __func__);
			goto done;
		}
		if(TS_MODE_CALL_STEP2 == mode_f.call) {
			si->si_ofs.max_tchs = OEM_LIMIT_EVENT_NUM_IN_CALL;
		} else {
	...
	/* limit event num during a call & proximity detection */
	default:
		dev_dbg(dev, "%s: unknown command=0x%08x\n",__func__,cmd);
		break;
	}
done:
	return rc;
				 ...
				 unsigned long event, void *data)
{
	...
	dev_dbg(cd->dev, " %s: change LCD status\n",__func__);
	if (evdata && evdata->data && event == FB_EVENT_BLANK && cd) {
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK){
			mutex_lock(&cd->state_lock);
			cyttsp4_change_state(cd->dev, (cd->oem_state & ~OEM_STATE_SLEEP_BIT));
			mutex_unlock(&cd->state_lock);
		} else if (*blank == FB_BLANK_POWERDOWN){
			mutex_lock(&cd->state_lock);
			cyttsp4_change_state(cd->dev, (cd->oem_state & ~OEM_STATE_SLEEP_BIT) | OEM_STATE_SLEEP_BIT);
			mutex_unlock(&cd->state_lock);
		}
	}
...
static int cyttsp4_ts_open(struct inode *inode, struct file *file)
{
	struct cyttsp4_core_data *data =
		container_of(inode->i_cdev, struct cyttsp4_core_data, device_cdev);

	file->private_data = data;
	return 0;
};

static int cyttsp4_ts_release(struct inode *inode, struct file *file)
{
	return 0;
};
const struct file_operations cyttsp4_ts_fops = {
	.owner = THIS_MODULE,
	.open = cyttsp4_ts_open,
	.unlocked_ioctl = cyttsp4_ts_ioctl,
	.release = cyttsp4_ts_release,
};
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	cd->touchmode_enable = CY_CMD_TOUCHMODE_AUTO_GLOVE; /* change finger/glove mode */

	/* Initialize mutexes and spinlocks */
	mutex_init(&cd->system_lock);
	mutex_init(&cd->adap_lock);
	mutex_init(&cd->state_lock); /* for status management */
	...
	INIT_WORK(&cd->touchmode_work, cyttsp4_touchmode_work);
	...
	cd->irq_enabled = true;
	enable_irq_wake(cd->irq); /* waiting lights LCD during a call */
	...
	err = fb_register_client(&cd->fb_notif);
	if (err)
	...
	ts_ctrl_init(&(cd->device_cdev), &cyttsp4_ts_fops);
	dev_dbg(dev, "%s: ok\n", __func__);
	return 0;
...
#if defined(CONFIG_FB)
	pr_info("%s: unregister fb_notifier\n",__func__);
...
static struct cyttsp4_core_driver cyttsp4_core_driver = {
	...
	.request_detect_finger = cyttsp4_detect_finger_, /* change finger/glove mode */
	.request_detect_large = cyttsp4_detect_large_, /* detect suppression */
