...
#include <linux/wakelock.h> /* waiting lights LCD during a call */

static struct wake_lock touchpanel_wake_lock; /* waiting lights LCD during a call */
/* prevent double touch when change mode to finger from glove */
#define CYTTSP4_MT_GLOVE_REL_WAIT msecs_to_jiffies(70)
static DEFINE_SPINLOCK(cyttsp4_mt_lock);
...
/* prevent double touch when change mode to finger from glove */
static void cyttsp4_pending_event_timer(unsigned long handle)
{
	struct cyttsp4_mt_data *md = (struct cyttsp4_mt_data *)handle;

	spin_lock(&cyttsp4_mt_lock);
	cyttsp4_get_mt_touches(md, 0);
	md->pending_event_flag = false;
	spin_unlock(&cyttsp4_mt_lock);
	return;
}
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_rec)
{
	...
	for (i = 0; i < num_cur_rec; i++) {
		...
		&& (md->pending_event_flag == true)) {
			del_timer(&md->pend_e_timer);
			md->pending_event_flag = false;
			md->pending_f_sync_flag = true;
			continue;
		}
		...
		&& (md->pending_event_flag == false)) {
			md->pending_event_flag =true;
			md->pending_f_sync_flag = true;
			mod_timer(&md->pend_e_timer, jiffies + CYTTSP4_MT_GLOVE_REL_WAIT);
			continue;
		}
		/* prevent double touch when change mode to finger from glove */
		...
		for (j = 0; j <= CY_ABS_W_OST ; j++) {
			sig = md->pdata->frmwrk->abs[((CY_ABS_X_OST + j) *
				CY_NUM_ABS_SET) + 0];
			if(sig == ABS_MT_PRESSURE && tch.abs[CY_TCH_X + j] > MAX_CYT_PRESSURE)
				tch.abs[CY_TCH_X + j] = MAX_CYT_PRESSURE;
			if (sig != CY_IGNORE_VALUE)
		...
		if (IS_TTSP_VER_GE(si, 2, 3)) {
			...
			for (j = 0; j < 2; j++) {
				...
				if(sig == ABS_MT_TOUCH_MAJOR){
					if( tch.abs[CY_TCH_MAJ + j]		> MAX_CYT_DIAMETER)
						tch.abs[CY_TCH_MAJ + j]		= MAX_CYT_DIAMETER;
					if( tch.abs[CY_TCH_MAJ + j + 1]	> MAX_CYT_DIAMETER)
						tch.abs[CY_TCH_MAJ + j + 1] = MAX_CYT_DIAMETER;
				}
				if (sig != CY_IGNORE_VALUE)
					input_report_abs(md->input, sig,
...
cyttsp4_get_mt_touches_pr_tch:
	...
	{
		if(md->pending_f_sync_flag == false) { /* prevent double touch when change mode to finger from glove */
		md->mt_function.final_sync(md->input,
			si->si_ofs.tch_abs[CY_TCH_T].max, mt_sync_count, ids);
		/* prevent double touch when change mode to finger from glove */
		} else {
			md->pending_f_sync_flag = false;
		}
	}
...
static int cyttsp4_xy_worker(struct cyttsp4_mt_data *md)
{
	...
	dev_vdbg(dev, "%s: extract data num_cur_rec=%d\n", __func__,
		num_cur_rec);
	if (num_cur_rec) {
		spin_lock_bh(&cyttsp4_mt_lock);/* prevent double touch when change mode to finger from glove */
		cyttsp4_get_mt_touches(md, num_cur_rec);
		spin_unlock_bh(&cyttsp4_mt_lock);/* prevent double touch when change mode to finger from glove */
	} else if (md->pending_event_flag == false) /* prevent double touch when change mode to finger from glove */
	{
		cyttsp4_lift_all(md);
	}
...
static int cyttsp4_mt_attention(struct cyttsp4_device *ttsp)
{
	...
	int temp_check_finger = 0; /* change finger/glove mode */
	 /* detect suppression */
	...
	if (!md->is_suspended) {
		...
		wake_lock_timeout(&touchpanel_wake_lock, 2 * HZ); /* waiting lights LCD during a call  */
		temp_check_finger = md->check_finger_touch; /* change finger/glove mode */
		temp_check_large = md->check_large_touch; /* detect suppression */
		rc = cyttsp4_xy_worker(md);
		/* change finger/glove mode */
		if (!temp_check_finger && md->check_finger_touch){
			dev_dbg(dev, "%s detect 1st finger touch\n", __func__);
			cyttsp4_request_detect_finger(ttsp);
		}
	 	/* detect suppression */
		if (temp_check_large && !md->check_large_touch) {
			md->request_glove_touch = true;
		}
		if (md->request_glove_touch && !GET_NUM_TOUCH_RECORDS(tt_stat)) {
			md->request_glove_touch = false;
			cyttsp4_request_detect_large(md->ttsp);
...
static int cyttsp4_mt_resume_attention(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	int rc = 0;
...
/* suspend/resume witch LCD */
	cyttsp4_subscribe_attention(ttsp, CY_ATTEN_SUSPEND,
		cyttsp4_mt_suspend_attention, 0);
	cyttsp4_subscribe_attention(ttsp, CY_ATTEN_RESUME,
...
/* suspend/resume witch LCD */
	cyttsp4_unsubscribe_attention(ttsp, CY_ATTEN_SUSPEND,
		cyttsp4_mt_suspend_attention, 0);
	cyttsp4_unsubscribe_attention(ttsp, CY_ATTEN_RESUME,
		cyttsp4_mt_resume_attention, 0);
...
static int cyttsp4_mt_rt_resume(struct device *dev)
{
	...
	mutex_lock(&md->report_lock);
	md->is_suspended = false;
	md->check_finger_touch = false; /* change finger/glove mode */
	mutex_unlock(&md->report_lock);
...
static int cyttsp4_mt_probe(struct cyttsp4_device *ttsp)
{
	...
	md->pdata = pdata;
	md->check_finger_touch = false; /* change finger/glove mode */
	md->check_large_touch = false; /* detect suppression */
	...
	wake_lock_init(&touchpanel_wake_lock, WAKE_LOCK_SUSPEND, dev_name(dev)); /* waiting lights LCD during a call */

	/* prevent double touch when change mode to finger from glove */
	setup_timer(&md->pend_e_timer, cyttsp4_pending_event_timer,
		(unsigned long)md);
	dev_dbg(dev, "%s: OK\n", __func__);
	return 0;
