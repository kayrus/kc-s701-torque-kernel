...
#include <mach/kc_board.h> /* check Bootloader(memory map) version */
...
struct cyttsp4_loader_data {
	...
	struct completion builtin_bin_cfg_complete; /* auto upgrade ttconfig */
	...
	bool config_loading;
#endif
	int upgrade_status;/* force calibration */
};
		...
		const struct firmware *fw)
{
	...
	if (!data->si) {
		dev_info(dev, "%s: No firmware infomation found, device FW may be corrupted\n",
			__func__);
		data->upgrade_status = 1; /* force FW upgrade when FW corrupted */
		return CYTTSP4_AUTO_LOAD_FOR_CORRUPTED_FW;
	}
	...
	upgrade = cyttsp4_check_firmware_version(ttsp, fw_ver_new,
			fw_revctrl_new_h, fw_revctrl_new_l);
/* force calibration */
	data->upgrade_status = upgrade;
	printk("%s: fw upgrade status %d\n",__func__,upgrade);
	if (upgrade >= -2)
		return 1;
...
static int upgrade_firmware_from_builtin(struct cyttsp4_device *ttsp)
{
	...
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	int retval;
	unsigned char *_fwname[] = {
		CY_FW_FILE_01_NAME,
		CY_FW_FILE_02_NAME,
	};
	unsigned char *fwname = _fwname[0];
...
/* auto upgrade ttconfig */
static void _cyttsp4_ttconfig_cont_builtin(const struct firmware *fw,
		void *context)
{
	struct cyttsp4_device *ttsp = context;
	struct device *dev = &ttsp->dev;
	...
	if (fw == NULL) {
		...
		goto _cyttsp4_ttconfig_cont_builtin_exit;
	}
	...
	data->config_data = (u8 *)fw->data;
	data->config_size = fw->size;
	...
	cfg_ver_new = get_unaligned_le16(&data->config_data[0x13]);
	cfg_ver_img = data->si->ttconfig.version;
...
_cyttsp4_ttconfig_cont_builtin_exit:
	release_firmware(fw);
	data->config_data = NULL;
	data->config_size = 0;

	complete(&data->builtin_bin_cfg_complete);
}

static int upgrade_ttconfig_from_builtin(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	int retval;
