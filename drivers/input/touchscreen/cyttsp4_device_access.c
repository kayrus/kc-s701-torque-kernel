...
/* suspend/resume with LCD */
static int cyttsp4_device_access_suspend_attention(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	int rc = 0;
	...
	cyttsp4_device_access_suspend(dev);
	return rc;
}
static int cyttsp4_device_access_resume_attention(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	int rc = 0;
	...
	cyttsp4_device_access_resume(dev);
	return rc;
}
...
/* suspend/resume with LCD */
	cyttsp4_subscribe_attention(ttsp, CY_ATTEN_SUSPEND, cyttsp4_device_access_suspend_attention, 0);
	cyttsp4_subscribe_attention(ttsp, CY_ATTEN_RESUME, cyttsp4_device_access_resume_attention, 0);
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_DEVICE_ACCESS_API
...
/* suspend/resume with LCD */
	cyttsp4_unsubscribe_attention(ttsp, CY_ATTEN_SUSPEND, cyttsp4_device_access_suspend_attention, 0);
	cyttsp4_unsubscribe_attention(ttsp, CY_ATTEN_RESUME, cyttsp4_device_access_resume_attention, 0);
	dev_set_drvdata(dev, NULL);
	kfree(dad);
	return 0;
}
