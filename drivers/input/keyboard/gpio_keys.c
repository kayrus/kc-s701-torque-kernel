...
/*	const struct gpio_keys_button *button; */
	...
	struct timer_list volkey_timer;
...
static bool gpio_keys_smlpress_check(struct gpio_button_data *bdata, unsigned int code, bool state)
{
	int rp_code;
	int vs_code;
	...
	if(volkey_stat[rp_code].state != volkey_stat[vs_code].state) {
		mod_timer(&bdata->volkey_timer,
			jiffies + msecs_to_jiffies(300));
	} else {
		del_timer_sync(&bdata->volkey_timer);
	}
...
/* 	const struct gpio_keys_button *button = bdata->button; */
	...
	if (g_kdm_check) {
		...
		if(gpio_keys_smlpress_check(bdata, button->code, !!state)) return;
					 ...
					 struct gpio_keys_button *button)
{
	...
	setup_timer(&bdata->volkey_timer,
