...
#define USB_SUSPEND_WAKE_LOCK_TIMEOUT_TIME    (msecs_to_jiffies(oem_chg_get_hkadc_monitor_time_ms() + 100))

#define USB_SUSPEND_DELAY_TIME	(500 * HZ/1000) /* 500 msec */

#define kc_dbg(instance, format, arg...)			\
	if (msm_otg_debug_log)							\
		dev_info(instance , format , ## arg)

bool msm_otg_debug_log = false;
module_param(msm_otg_debug_log, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(msm_otg_debug_log, "Enable KC Log Output");
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	if ((test_bit(B_SESS_VLD, &motg->inputs) && !device_bus_suspend &&
		...
		dev_info(phy->dev, "USB is busy. B_VLD(%d) type(%d) A_BUS(%d)\n",
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	if (motg->chg_type == USB_SDP_CHARGER)
		...
		tmp_charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
	...
	dev_info(motg->phy.dev, "setting usb power supply type %s(%d)\n",
			chg_to_string(motg->chg_type), charger_type);
...
static void msm_ta_detect_work(struct work_struct *w)
{
	...
	if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS) {
		...
		motg->cur_power = 0;
		motg->dpdm_line_state = PORTSC_LS;
		msm_otg_start_peripheral(otg, 0);
...
static void msm_chg_detect_work(struct work_struct *w)
{
	...
	/* Connection check */
	if ((atomic_read(&motg->usb_det) == 0) && (atomic_read(&motg->pma_det) == 0)) {
		motg->chg_state = USB_CHG_STATE_UNDEFINED; /* fail safe */
		return;
	}
	...
	case USB_CHG_STATE_WAIT_FOR_DCD:
		...
		kc_dbg(motg->phy.dev, "= CHG DET I = : is_dcd = %d, tmout = %d\n",
			is_dcd, tmout);
		if (is_dcd || tmout || msm_otg_connect_chg_pma_only(motg)) {
			if (is_dcd)
				dcd = true;
			else
				dcd = false;
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		motg->dpdm_line_state = line_state;

		dev_dbg(motg->phy.dev, "detect_work USB (D+:D-)=(%d:%d) USB_PORTSC=(0x%x)\n",
			(line_state & PORTSC_LS_DP)? 1 : 0,
			(line_state & PORTSC_LS_DM)? 1 : 0,
		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_PROPRIETARY_CHARGER:
					dev_dbg(motg->phy.dev, "sm_work USB (D+:D-)=(%d:%d)\n",
						(motg->dpdm_line_state & PORTSC_LS_DP)? 1 : 0,
						(motg->dpdm_line_state & PORTSC_LS_DM)? 1 : 0);
					if (msm_otg_connect_chg_pma_only(motg)) {
						/* PMA charg */
						msm_otg_notify_charger(motg, IDEV_PMA_CHG_MAX);
					} else if (motg->dpdm_line_state){
						/* D+D- abnormality */
...
void msm_otg_change_charge_device(int usb_det, int pma_det)
{
	...
	if(start_up_flag) {
		start_up_flag = false;
		if ((usb_det == 0) && (pma_det == 0)) {
			msm_otg_set_vbus_state(0);
			return;
		}
	}

	/* HW specification. ignore this case. */
	if ((usb_det == 1) && (pma_det == 0))
		return;
	...
	if (usb_det && (atomic_read(&motg->pma_det) == 1)) {
				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		dev_info(motg->phy.dev, "POWER_SUPPLY_PROP_PRESENT vbus set connect = %d\n"
									, val->intval);
		/* USB charge was cut? */
		if ((!val->intval) && (atomic_read(&motg->usb_det) == 1)) {
