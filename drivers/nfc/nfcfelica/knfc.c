...
/* NFC_VFC_RST device */
#define MFD_NFC_VFC_RST_GPIO_NUM		120
...
static int intu_poll_open(struct inode *inode, struct file *file)
{
	...
	ret = request_irq(gpio_to_irq(INTU_POLL_GPIO_NO), intu_poll_irq_handler, irqflag, INTU_POLL_DEV_NAME, (void*)intu_d);
	if (ret) {
		intu_d->open_flag = 0;
