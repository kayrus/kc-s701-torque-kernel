...
#define YAS_BMA250_POWERMODE_250E_SUS                                          4
...
static int yas_bma250_read_reg(unsigned char, unsigned char *, int);
static int yas_bma250_write_reg(unsigned char, unsigned char *, int);
static unsigned char yas_bma250_read_reg_byte(unsigned char);
...
static int yas_bma250_read_reg(unsigned char adr, unsigned char *buf, int len)
{
...
static int yas_bma250_write_reg(unsigned char adr, unsigned char *buf, int len)
{
	struct yas_acc_driver_callback *cbk = &pcb->callback;
	int err;
...
static unsigned char yas_bma250_read_reg_byte(unsigned char adr)
{
...
static int yas_bma250_set_delay(int delay)
{
	...
	if (yas_bma250_get_enable()) {
		if (acc_data.chip_id == YAS_BMA250_CHIP_ID_250E ||
			...
			yas_bma250_update_bits(YAS_BMA250_POWERMODE_250E,
					       YAS_BMA250_POWERMODE_250E_SUS
					       );
	        usleep_range(450,450);
			yas_bma250_write_reg_byte(0x3E, 0x00);
