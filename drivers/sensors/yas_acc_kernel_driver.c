...
struct yas_acc_private_data {
	...
	struct mutex mtp_recovery_mutex;
...
static int yas_acc_mtp_recovery_device_check(uint8_t *i2c_addr)
{
  ...
  {
    if (yas_acc_i2c_read_ex(addr[i], 0x00, &buf, 1))
    {
      err = -1;
    }
    else
...
static int yas_acc_mtp_recovery_set_correct_addr(uint8_t *i2c_addr)
{
  ...
  int err = 0;
  yas_acc_i2c_read_ex(addr, 0x11, &tmpbuf, 1);
  buf = 0x80 | (tmpbuf & ~0xe0);
  yas_acc_i2c_write_ex(addr, 0x11, &buf, 1);
  usleep_range(450, 450);
  buf = 0x00;
  yas_acc_i2c_write_ex(addr, 0x3E, &buf, 1);
...
static int yas_acc_mtp_recovery(uint8_t *backup_data)
{
  uint8_t i2c_addr = 0;
  int err = 0;
  uint8_t tmpbuf = 0;
  ...
  else
  {
    ...
    {
      memset((void*)backup_data, 0, sizeof(uint8_t)*32 );
      printk("accelerometer get backup data failed.\n");
    }
  }
  yas_acc_i2c_read_ex(i2c_addr, 0x11, &tmpbuf, 1);
  if(yas_acc_get_enable(data->driver))
  {
    buf = 0x40 | (tmpbuf & ~0xe0);
  }
  else
  {
    buf = 0x20 | (tmpbuf & ~0xe0);
  }
  yas_acc_i2c_write_ex(i2c_addr, 0x11, &buf, 1);
  usleep_range(450,450);
...
static int yas_acc_set_enable(struct yas_acc_driver *driver, int enable)
{
	...
	if (yas_acc_get_enable(data->driver) != enable) {
		if (enable) {
            ...
            mutex_lock(&data->mtp_recovery_mutex);
            if(data->mtp_recovery[31] == 1) {
                yas_acc_mtp_recovery(data->mtp_recovery);
            }
            mutex_unlock(&data->mtp_recovery_mutex);
...
#ifdef SUPPORT_ACC_MTP_RECOVERY
static ssize_t yas_acc_mtp_recovery_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct input_dev *input = to_input_dev(dev);
...
static ssize_t yas_acc_mtp_recovery_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf,
				    size_t count)
{
	...
	if(data->mtp_recovery[31] == 1){
		YLOGE(("%s(): already bukup mtp_recovery\n", __func__));
		mutex_unlock(&data->mtp_recovery_mutex);
		return count;
	}
...
#ifdef SUPPORT_ACC_MTP_RECOVERY
static DEVICE_ATTR(mtp_recovery,
		   S_IRUGO|S_IWUSR,
		   yas_acc_mtp_recovery_show,
		   yas_acc_mtp_recovery_store
		   );
#endif /* SUPPORT_ACC_MTP_RECOVERY */
...
#if SUPPORT_ACC_MTP_RECOVERY
	memset(data->mtp_recovery, 0, sizeof(data->mtp_recovery));
	mutex_init(&data->mtp_recovery_mutex);
#endif /* SUPPORT_ACC_MTP_RECOVERY */
...
static struct of_device_id yas_acc_match_table[] = {
	{ .compatible = YAS_ACC_KERNEL_NAME,},
	{ },
};
...
static struct i2c_driver yas_acc_driver = {
	.driver = {
		.name = "accelerometer",
		.owner = THIS_MODULE,
		.of_match_table = yas_acc_match_table,
	},
	.probe = yas_acc_probe,
