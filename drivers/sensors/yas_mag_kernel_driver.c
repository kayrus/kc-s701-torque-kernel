...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct yas_mag_filter filter;
    struct yas_matrix pp1_static_matrix     = {YAS_PP1_MAG_STATIC_MATRIX};
    uint32_t min_uV=0,max_uV=0,load_uA=0;

    of_property_read_u32(client->dev.of_node, "geomagnetic-vdd-min-voltage", &min_uV);
    of_property_read_u32(client->dev.of_node, "geomagnetic-vdd-max-voltage", &max_uV);
    of_property_read_u32(client->dev.of_node, "geomagnetic-vdd-load-current", &load_uA);
    printk(KERN_NOTICE "[MAG]%s regulator min_uV = %d, max_uV = %d, load_uA = %d\n",
        __func__, min_uV, max_uV, load_uA);

    sensor_vdd = regulator_get(&client->dev, "geomagnetic-vdd");
    if( IS_ERR(sensor_vdd) ) {
        printk(KERN_ERR "[MAG]%s regulator_get fail.\n", __func__);
        rt = PTR_ERR(sensor_vdd);
        goto reguget_err;
    }

    rt = regulator_set_voltage(sensor_vdd, min_uV, max_uV);
    if( rt ) {
        printk(KERN_ERR "[MAG]%s regulator_set_voltage fail. err=%d\n", __func__, rt);
        goto vdd_err;
    }

    rt = regulator_set_optimum_mode(sensor_vdd, load_uA);
    if( rt < 0 ) {
        printk(KERN_ERR "[MAG]%s regulator_set_optimum_mode fail. err=%d\n", __func__, rt);
        goto vdd_err;
    }

    rt = regulator_enable(sensor_vdd);
    if( rt ) {
        printk(KERN_ERR "[MAG]%s regulator_enable fail. err=%d\n", __func__, rt);
        goto vdd_err;
    }
	...
	data->distortion[0] = YAS_WS2_1_MAGCALIB_DISTORTION_1;
	data->distortion[1] = YAS_WS2_1_MAGCALIB_DISTORTION_2;
	data->distortion[2] = YAS_WS2_1_MAGCALIB_DISTORTION_3;
	data->shape = 0;
	...
	if (hwdriver.set_static_matrix != NULL) {
		rt = hwdriver.set_static_matrix(&pp1_static_matrix);
		if (rt < 0) {
			YLOGE(("hwdriver.set_static_matrix() failed[%d]\n", rt));
			goto err;
		}
	}
...
static struct of_device_id geomagnetic_match_table[] = {
	{ .compatible = GEOMAGNETIC_I2C_DEVICE_NAME,},
	{ },
};
...
static struct i2c_driver geomagnetic_i2c_driver = {
	.driver = {
		...
		.owner		= THIS_MODULE,
		.of_match_table = geomagnetic_match_table,
	},
