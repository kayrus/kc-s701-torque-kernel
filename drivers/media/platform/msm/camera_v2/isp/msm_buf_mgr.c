...
static bool g_data_abort_error_ind = false;
	...
	uint32_t buf_handle)
{
	...
	for (i = 0; i < bufq->num_bufs; i++) {
		...
		if ((buf_info->num_planes == 2)
		&&  (buf_info->mapped_info[0].handle != buf_info->mapped_info[1].handle)) {
			if (g_data_abort_error_ind == false) {
				g_data_abort_error_ind = true;
				pr_err("%s: Data Abort Error\n", __func__);
				return -EFAULT;
			} else {
				return rc;
			}
	...
	uint32_t bufq_handle)
{
	...
	rc = msm_isp_buf_unprepare(buf_mgr, bufq_handle);
...
static int msm_isp_release_all_bufq(
	struct msm_isp_buf_mgr *buf_mgr)
{
	struct msm_isp_bufq *bufq = NULL;
	int i;
	...
	for (i = 0; i < buf_mgr->num_buf_q; i++) {
		...
		if (!bufq->bufq_handle)
			continue;
		rc = msm_isp_buf_unprepare(buf_mgr, bufq->bufq_handle);
		if (rc == -EFAULT) {
			data_abort = true;
		}
	...
	struct msm_isp_buf_mgr *buf_mgr)
{
	...
	if (--buf_mgr->open_count)
		return 0;
	rc = msm_isp_release_all_bufq(buf_mgr);
