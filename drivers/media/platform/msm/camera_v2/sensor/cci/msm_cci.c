...
#define CHECK_AND_INIT_COMPLETION(completion)	\
{												\
	if (completion.done) {						\
		pr_notice("%s:%d completion already done(%d)."		\
			" init completion.\n", __func__, __LINE__, completion.done);	\
	...
	enum cci_i2c_master_t master)
{
	...
	CHECK_AND_INIT_COMPLETION(cci_dev->cci_master_info[master].reset_complete);
