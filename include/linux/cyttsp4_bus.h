...
struct cyttsp4_core_driver {
	...
	int (*request_disable_scan_type)(struct cyttsp4_device *ttsp,
			u8 scan_type);
	void (*request_detect_finger)(struct cyttsp4_device *ttsp); /* change finger/glove mode */
	void (*request_detect_large)(struct cyttsp4_device *ttsp); /* detect suppression */
...
static inline void cyttsp4_request_detect_finger(struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *cd = ttsp->core;
	struct cyttsp4_core_driver *d = to_cyttsp4_core_driver(cd->dev.driver);
	d->request_detect_finger(ttsp);
}
/* detect suppression */
static inline void cyttsp4_request_detect_large(struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *cd = ttsp->core;
	struct cyttsp4_core_driver *d = to_cyttsp4_core_driver(cd->dev.driver);
