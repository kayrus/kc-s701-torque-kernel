...
static int wcd9xxx_setup_jack_detect_irq(struct wcd9xxx_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->gpio) {
#ifdef CONFIG_KYOCERA_MSND_HS_GPIO
		wake_lock_init(&mbhc->hs_det_wlock, WAKE_LOCK_SUSPEND, "gpio_hs_det");

		ret = request_threaded_irq(mbhc->mbhc_cfg->gpio_irq, NULL,
					   gpio_jack_detect_on_gpio_irq,
					   (IRQF_TRIGGER_RISING |
		...
		ret = request_threaded_irq(mbhc->mbhc_cfg->sw_gpio_irq, NULL,
					   gpio_btn_detect_on_gpio_irq,
					   (IRQF_TRIGGER_RISING |
					    IRQF_TRIGGER_FALLING),
					   "Jack SW Detect", mbhc);
		if (ret) {
			pr_err("%s: Failed to request gpio irq %d\n", __func__,
			       mbhc->mbhc_cfg->sw_gpio_irq);
		} else {
			disable_irq(mbhc->mbhc_cfg->sw_gpio_irq);
		}

		gpio_set_hs_det_proc_state_init(mbhc);
		      ...
		      bool impedance_det_en)
{
	...
	spin_lock_init(&mbhc->hs_spinlock);
#endif
...
void wcd9xxx_mbhc_deinit(struct wcd9xxx_mbhc *mbhc)
{
	...
	wake_lock_destroy(&mbhc->hs_det_wlock);
#endif
}
