...
#ifdef CONFIG_KYOCERA_MSND
static int kc_msm8226_ext_rcv_pamp;
...
#ifdef CONFIG_KYOCERA_MSND
static int kc_ext_rcv_amp_gpio_on   = -1;
...
static void kc_msm8226_ext_rcv_power_amp_enable(u32 enable)
{
	if (enable) {
		...
		pr_debug("%s: Disabled RCV_ON  (MSM8926 GPIO[%d])\n",__func__, kc_ext_rcv_amp_gpio_on);
		gpio_direction_output(kc_ext_rcv_amp_gpio_on, 0);
	}
...
static void kc_msm8226_ext_rcv_power_amp_on(u32 rcv)
{
	if (gpio_is_valid(kc_ext_rcv_amp_gpio_on)){
		if (rcv & KC_RCVAMP_ENABLE) {
			pr_debug("%s:Enable left and right receiver case spk = 0x%x\n",
				__func__, rcv);

			kc_msm8226_ext_rcv_pamp |= rcv;

			if (kc_msm8226_ext_rcv_pamp & KC_RCVAMP_ENABLE) {
				pr_debug("%s  enable power", __func__);
				kc_msm8226_ext_rcv_power_amp_enable(1);
			}
		} else  {
...
static void kc_msm8226_ext_rcv_power_amp_off(u32 rcv)
{
	if (gpio_is_valid(kc_ext_rcv_amp_gpio_on)) {
		if (rcv & KC_RCVAMP_ENABLE) {
			...
			if (!kc_msm8226_ext_rcv_pamp) {
				pr_debug("%s  disable power", __func__);
				kc_msm8226_ext_rcv_power_amp_enable(0);
				kc_msm8226_ext_rcv_pamp = 0;
			}
		 } else  {
...
static int kc_msm8226_ext_rcvamp_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
			kc_msm8226_ext_rcv_power_amp_on(KC_RCVAMP_ENABLE);
		} else {
        ...
        if (!strncmp(w->name, "Ext Rcv Pos", 11)) {
			kc_msm8226_ext_rcv_power_amp_off(KC_RCVAMP_ENABLE);
		} else {
			pr_err("%s() Invalid Receiver Widget = %s\n",
				__func__, w->name);
			return -EINVAL;
...
#ifdef CONFIG_KYOCERA_MSND
	SND_SOC_DAPM_SPK("Ext Rcv Pos", kc_msm8226_ext_rcvamp_event),
...
static __devinit int msm8226_asoc_machine_probe(struct platform_device *pdev)
{
	...
	{
#else
	if (OEM_get_board() != OEM_BOARD_WS0_TYPE) {
#endif
		kc_ext_rcv_amp_gpio_on = of_get_named_gpio(pdev->dev.of_node,
					"kc,rcv-amp-gpio-on", 0);
		if (kc_ext_rcv_amp_gpio_on < 0) {
			dev_err(&pdev->dev, "Looking up %s propertyd %d\n", pdev->dev.of_node->full_name, kc_ext_rcv_amp_gpio_on);
		} else {
			ret = gpio_request(kc_ext_rcv_amp_gpio_on, "RECEIVER_AMP_ON");
			if (ret) {
				dev_err(card->dev,
...
#ifdef CONFIG_KYOCERA_MSND_HS_GPIO
	jack_gpio = of_get_named_gpio(pdev->dev.of_node,
				"kc,jack-gpios", 0);
	if (jack_gpio < 0) {
		pr_err("%s: jack of_get_named_gpio %d failed \n", __func__,
		       jack_gpio);
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed %d\n",
			"kc, jack-gpios", pdev->dev.of_node->full_name,
			jack_gpio);
		ret = -ENODEV;
		goto err;
	}

	jack_sw_gpio = of_get_named_gpio(pdev->dev.of_node,
				"kc,jack-sw-gpios", 0);
	if (jack_sw_gpio < 0) {
		pr_err("%s: jack sw of_get_named_gpio %d failed \n", __func__,
		       jack_sw_gpio);
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed %d\n",
			"kc, jack-gpios", pdev->dev.of_node->full_name,
	...
	mbhc_cfg.gpio = jack_gpio;
	mbhc_cfg.gpio_irq = gpio_to_irq(mbhc_cfg.gpio);
	mbhc_cfg.sw_gpio = jack_sw_gpio;
	mbhc_cfg.sw_gpio_irq = gpio_to_irq(mbhc_cfg.sw_gpio);

	ret = gpio_request(jack_sw_gpio, "HEADSET_SW");
	if (ret < 0) {
	...
	mbhc_cfg.calibration = def_tapan_mbhc_cal();
	if (mbhc_cfg.calibration) {
		ret = tapan_hs_detect(g_rtd->codec, &mbhc_cfg);
	} else {
		ret = -ENOMEM;
		goto err;
	}
...
#ifdef CONFIG_KYOCERA_MSND
	if (kc_ext_rcv_amp_gpio_on >= 0) {
		gpio_free(kc_ext_rcv_amp_gpio_on);
		kc_ext_rcv_amp_gpio_on = -1;
	}
	if (kc_ext_spk_amp_gpio_ctrl >= 0) {
...
#ifdef CONFIG_KYOCERA_MSND_HS_GPIO
	if (jack_gpio > 0) {
		dev_dbg(&pdev->dev, "%s free jack_gpio %d\n",
			__func__, jack_gpio);
		gpio_free(jack_gpio);
		jack_gpio = 0;
	}
	if (jack_sw_gpio > 0) {
		dev_dbg(&pdev->dev, "%s free jack_sw_gpio gpio %d\n",
			__func__, jack_sw_gpio);
		gpio_free(jack_sw_gpio);
		jack_sw_gpio = 0;
	}
#endif
